...
class Drum {
  ...
  _highlight() {
    this.list.querySelectorAll('.drum-item').forEach(el => {
      ...
      const d = Math.min(Math.abs(v - this.val), this.count - Math.abs(v - this.val));
...
const fmt = m => { m = wrap1440(m); return `${String(Math.floor(m/60)).padStart(2,'0')}:${String(m%60).padStart(2,'0')}`; };
const dur = m => { const h=Math.floor(m/60), r=m%60; return h&&r?`${h}h${String(r).padStart(2,'0')}`:h?`${h}h`:`${r}min`; };
...
document.getElementById('calc-btn').addEventListener('click', () => {
  ...
  if (innerWidth >= 860 && !main.classList.contains('revealed')) {
  ...
  inner.insertAdjacentHTML('beforeend', `
    <div class="ideal-box">
      ...
      <div>
        ...
        <div class="ideal-note">${IDEAL_N + 1} cycles · ${dur((IDEAL_N + 1) * CYCLE)} de sommeil</div>
      </div>
    </div>
...
float line(vec2 uv, float off){
  ...
  vec2 d=uv-M; y+=(M.y-uv.y)*exp(-dot(d,d)*5.0)*(-0.5)*BI;
  return 0.018/max(abs(uv.y-y)+0.008,0.001);
}

void main(){
  vec2 uv=(2.0*gl_FragCoord.xy-R)/R.y; uv.y*=-1.0; uv+=PO;
  float L=length(uv)+0.001;
  vec3 col=vec3(0.0);
  col+=palette(0.0/5.0)*line(uv*rot(-log(L))+vec2(2.00,-0.7),1.5)*0.2;
  ...
  col+=palette(0.0/5.0)*line(uv*rot(0.2*log(L))+vec2(5.00,0.0),2.00);
  ...
  vec2 ruv;
  ruv=uv*rot(-0.4*log(L));ruv.x*=-1.0;col+=palette(0.0/5.0)*line(ruv+vec2(10.00,0.5),1.0)*0.1;
  ...
  const uT=gl.getUniformLocation(prog,'T'), uR=gl.getUniformLocation(prog,'R'),
        uM=gl.getUniformLocation(prog,'M'), uBI=gl.getUniformLocation(prog,'BI'),
        uPO=gl.getUniformLocation(prog,'PO');

  let tMx=-1000,tMy=-1000,cMx=-1000,cMy=-1000,tBI=0,cBI=0,tPx=0,tPy=0,cPx=0,cPy=0;
  const D=0.05, PS=0.2;
  ...
  window.addEventListener('mousemove',e=>{
    ...
    tBI=1; tPx=(e.clientX/innerWidth-0.5)*PS; tPy=-(e.clientY/innerHeight-0.5)*PS;
  });
  ...
  (function loop(ts){
    ...
    cMx+=(tMx-cMx)*D; cMy+=(tMy-cMy)*D; cBI+=(tBI-cBI)*D;
    cPx+=(tPx-cPx)*D; cPy+=(tPy-cPy)*D;
    gl.uniform1f(uT,t); gl.uniform2f(uR,canvas.width,canvas.height);
    gl.uniform2f(uM,cMx,cMy); gl.uniform1f(uBI,cBI); gl.uniform2f(uPO,cPx,cPy);
